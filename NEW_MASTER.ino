...
//const char string_0[] PROGMEM = "Switch off the machine, insert the sim card and try again.            ";
//char buffer[70];
...
void loop() 
{
  ...
  if (AddressableModeActive == 1 && ConventionalModeActive == 1) // Addressable Mode is Active
  {
    bool pkey=ProgModeKeyCheck ();
    if(pkey==1)
    {
    ...
    if(check==DecVal)
    {
     ...
     Serial.print(ValidTrigger1);
      ...
      lcd.print(F("Sensor Triggered"));
        ...
        if(SimPresent==1)
        {
        (String(S_Name),String(tval),num_set, voice_msg);
        }  
        
        //Serial.println(F("Trigger is valid"));
        String log_data="";
...
void ProgrammingMode()
{
  ...
  if(i==1)
  {
    ...
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(F("Select devc mode"));
    lcd.setCursor(0,1);
    lcd.print(F("Cnv-1 Ad-2 Hyb-3"));
